...
class BenchmarkRunner:
    ...
    def _run_dnsperf(
        self, server: str, port: int, duration: int, qps: int, warmup: bool = False
    ) -> Tuple[str, Dict]:
        cmd = [
            "dnsperf",
            "-s",
    ...
    def _format_cache_stats(self, stats: Dict[str, Any]):
        ...
        # Display Server + Resolver side by side
        if len(panels) == 2:
        ...
        # NS Cache, Delegation Cache, and Address Cache side by side
    ...
    def _format_perf_report(self, report_path: Path) -> None:
        ...
        # Build root index map
        root_indices = []
        last_root_idx = -1
        for i, func in enumerate(functions):
            if func.get("depth", 0) == 0:
                last_root_idx = i
            root_indices.append(last_root_idx)

        # Build tree structure with table format
        table = Table(
    ...
    def run(self) -> int:
        ...
        try:
            ...
            _, self.pico_result = self._run_dnsperf(
                "127.0.0.1", self.port, self.duration, self.qps
            )
            ...
            if self._start_knot():
                ...
                _, self.knot_result = self._run_dnsperf(
                    "127.0.0.1", self.knot_port, self.duration, self.qps
                )
                self._cleanup()
